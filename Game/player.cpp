...
Player::Player(std::string id)
	: Game_Object(id, "Texture.Pinky.Idle")
{
	_speed = 0.1f;
...
void Player::simulate_AI(Uint32, Assets* assets, Input* input, Scene*)
{
	switch(_state.top())
	{
		...
		case State::Running:
			...
			break;
		case State::Punching:
			if (input->is_button_state(Input::Button::PUNCHING, Input::Button_State::PRESSED))
			{
				push_state(State::Punching, assets);
			}
			else if (input->is_button_state(Input::Button::PUNCHING, Input::Button_State::RELEASED))
			{
				pop_state(assets);
			}
			break;
...
void Player::handle_enter_state(State state, Assets* assets)
{
	switch(state)
	{
		...
		case State::Punching:
		{
			_texture_id = "Texture.Pinky.Punching";
			const int punching_channel = 3;
			Sound* sound = (Sound*)assets->get_asset("Sound.Punching");
			Mix_PlayChannel(punching_channel, sound->data(), -1);
			break;
		}
		case State::Death:
		{
			_texture_id = "Texture.Pinky.Death";
			const int punching_channel = 4;
...
void Player::handle_exit_state(State state, Assets*)
{
	switch(state)
	{
		...
		{
			const int punching_channel = 3;
			Mix_HaltChannel(punching_channel);
			break;
		}
	}
}